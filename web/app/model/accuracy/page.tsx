...
export const metadata = pageMeta({
  title: "NRL model accuracy — out-of-sample backtest",
  description:
    "How the NRL model actually performs: out-of-sample try-scorer AUC, calibration and a reliability curve, plus per-stat error versus a recent-form baseline. Seasons 2023–25 held out.",
  path: "/model/accuracy",
  keywords: ["NRL model accuracy", "NRL backtest", "NRL model calibration", "NRL prediction accuracy"],
});
...
export default async function AccuracyPage() {
  ...
  if (!t && !bt.regression.length)
    return <p style={{ color: "var(--muted)" }}>Backtest data isn’t available yet.</p>;
